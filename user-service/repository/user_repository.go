...
func (u *userRepository) DeleteUser(ctx context.Context, id uint) error {
	...
	if err := u.db.WithContext(ctx).Select("id", "name", "email", "password", "photo", "phone").Preload("Roles").Where("id = ?", id).First(&modelUser).Error; err != nil {
...
func (u *userRepository) EditAssignUserToRole(ctx context.Context, assignRoleID uint, UserID uint, roleID uint) error {
	...
	if err := u.db.WithContext(ctx).Select("id", "user_id", "role_id").Where("id = ?", assignRoleID).First(&userRole).Error; err != nil {
...
func (u *userRepository) GetAllUserRoles(ctx context.Context, page int, limit int, search string, sortBy string, sortOrder string) ([]model.UserRole, int64, error) {
	...
	if search != "" {
		query = query.Joins("JOIN users ON user_roles.user_id = users.id").Joins("JOIN roles ON user_roles.role_id = roles.id").Where("users.name ILIKE ? OR users.email ILIKE ? OR roles.name ILIKE ?", "%"+search+"%", "%"+search+"%", "%"+search+"%")
	}
...
func (u *userRepository) GetAllUsers(ctx context.Context, page int, limit int, search string, sortBy string, sortOrder string) ([]model.User, int64, error) {
	...
	if err := query.Select("id", "name", "email", "password", "photo", "phone", "created_at").Preload("Roles").Order(sortBy + " " + sortOrder).Limit(limit).Offset(offset).Find(&modelUsers).Error; err != nil {
...
func (u *userRepository) GetUserByEmail(ctx context.Context, email string) (*model.User, error) {
	...
	modelUsers := model.User{}
	if err := u.db.WithContext(ctx).Select("id", "name", "email", "password", "photo", "phone", "created_at").Where("email = ?", email).Preload("Roles").First(&modelUsers).Error; err != nil {
...
func (u *userRepository) GetUserByID(ctx context.Context, id uint) (*model.User, error) {
	...
	modelUsers := model.User{}
	if err := u.db.WithContext(ctx).Select("id", "name", "email", "password", "photo", "phone", "created_at").Where("id = ?", id).Preload("Roles").First(&modelUsers).Error; err != nil {
...
func (u *userRepository) GetUserByRoleName(ctx context.Context, roleName string) ([]model.User, error) {
	...
	subQuery := u.db.Table("user_role").Select("user_role.user_id").Joins("JOIN roles ON user_role.role_id = roles.id").Where("roles.name = ?", roleName)
...
func (u *userRepository) UpdateUser(ctx context.Context, user model.User) error {
	...
	if err := u.db.WithContext(ctx).Select("id", "name", "email", "password", "photo", "phone").Preload("Roles").Where("id = ?", user.ID).First(&modelUser).Error; err != nil {
