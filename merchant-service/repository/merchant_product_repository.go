...
func (m *merchantProductRepository) DeleteMerchantProduct(ctx context.Context, id uint) error {
	...
	default:
		...
		if err := m.db.WithContext(ctx).Where("id = ?", id).First(&modelMerchantProduct).Error; err != nil {
			log.Errorf("[MerchantProductRepository] DeleteMerchantProduct - 1: %v", err)
			return err
		}
...
func (m *merchantProductRepository) GetMerchantProductByID(ctx context.Context, id uint) (*model.MerchantProduct, error) {
	...
	default:
		...
		if err := m.db.WithContext(ctx).Where("id = ?", id).First(&merchantProduct).Error; err != nil {
			log.Errorf("[MerchantProductRepository] GetMerchantProductByID - 1: %v", err)
			return nil, err
		}
...
func (m *merchantProductRepository) GetMerchantProducts(ctx context.Context, page int, limit int, search string, sortBy string, sortOrder string, merchantID uint, productID uint) ([]model.MerchantProduct, int64, error) {
	...
	default:
		...
		offset := (page - 1) * limit
		if err := query.Order(sortBy + " " + sortOrder).WithContext(ctx).Preload("Merchant").Offset(offset).Limit(limit).Find(&modelMerchantProducts).Error; err != nil {
...
// GetMerchantProductsByProductIDAndMerchantID implements MerchantProductRepositoryInterface.
func (m *merchantProductRepository) GetMerchantProductsByProductIDAndMerchantID(ctx context.Context, productID uint, merchantID uint) (*model.MerchantProduct, error) {
	select {
	case <-ctx.Done():
		log.Errorf("[MerchantProductRepository] GetMerchantProductsByProductIDAndMerchantID - 1: %v", ctx.Err())
		return nil, ctx.Err()
	default:
		var merchantProduct model.MerchantProduct
		if err := m.db.WithContext(ctx).Where("product_id = ? AND merchant_id = ?", productID, merchantID).First(&merchantProduct).Error; err != nil {
			log.Errorf("[MerchantProductRepository] GetMerchantProductsByProductIDAndMerchantID - 1: %v", err)
			return nil, err
		}
...
func (m *merchantProductRepository) GetProductTotalStock(ctx context.Context, productID uint) (int, error) {
	...
	default:
		var totalStock int
		if err := m.db.WithContext(ctx).Model(&model.MerchantProduct{}).Where("product_id = ?", productID).Select("COALESCE(SUM(stock), 0)").Scan(&totalStock).Error; err != nil {
