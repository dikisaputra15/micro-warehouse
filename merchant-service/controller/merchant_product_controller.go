...
func (m *merchantProductController) GetMerchanProductByID(c *fiber.Ctx) error {
	...
	if err != nil {
		log.Errorf("[MerchantProductController] GetMerchanProductByID - 1: %v", err)
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
...
func (m *merchantProductController) GetMerchantProducts(c *fiber.Ctx) error {
	...
	merchantProducts, products, warehouses, total, err := m.merchantProductUsecase.GetMerchantProducts(ctx, req.Page, req.Limit, req.Search, req.SortBy, req.SortOrder, req.MerchantID,
	req.ProductID)
	if err != nil {
