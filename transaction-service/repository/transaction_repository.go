...
type TransactionRepositoryInterface interface {
	...
	UpdatePaymentStatus(ctx context.Context, orderID uint, paymentStatus, paymentMethod, transactionID, fraudStatus string) error
}
...
func (t *transactionRepository) GetDashboardStats(ctx context.Context) (int64, int64, int64, error) {
	...
	default:
		...
		err := t.db.WithContext(ctx).Model(&model.Transaction{}).Where("payment_status = ?", model.PaymentStatusSuccess).Select("COALESCE(SUM(grand_total), 0) as total_revenue, COUNT(*) as total_transactions").Scan(&result).Error
...
func (t *transactionRepository) GetDashboardStatsByMerchant(ctx context.Context, merchantID uint) (int64, int64, int64, error) {
	...
	default:
		...
		err := t.db.WithContext(ctx).Model(&model.Transaction{}).Where("merchant_id = ? AND payment_status = ?", merchantID, model.PaymentStatusSuccess).Select("COALESCE(SUM(grand_total), 0) as total_revenue, COUNT(*) as total_transactions").Scan(&result).Error
		...
		err = t.db.WithContext(ctx).Model(&model.TransactionProduct{}).Joins("JOIN transactions ON transactions.id = transaction_products.transaction_id").Where("transactions.merchant_id = ? AND transactions.payment_status = ?", merchantID, model.PaymentStatusSuccess).Select("COALESCE(SUM(transaction_products.quantity), 0) as products_sold").Scan(&ProductsSold).Error
...
func (t *transactionRepository) GetTransactions(ctx context.Context, page int, limit int, search string, sortBy string, sortOrder string, merchantID uint) ([]model.Transaction, int64, error) {
	...
	default:
		...
		baseSql := t.db.WithContext(ctx).Model(&model.Transaction{}).Preload("TransactionProducts").Preload("Merchant")
		...
		if merchantID != 0 {
			baseSql = baseSql.Where("transactions.merchant_id = ?", merchantID)
		}
		...
		err := baseSql.WithContext(ctx).Order(sortBy + " " + sortOrder).Offset(offset).Limit(limit).Find(&transactions).Error
...
func (t *transactionRepository) UpdatePaymentStatus(ctx context.Context, orderID uint, paymentStatus string, paymentMethod string, transactionID string, fraudStatus string) error {
	select {
	case <-ctx.Done():
